...
- (void)viewConfig
{
    ...
    _navgationTabBar.backgroundColor = [UIColor colorWithRed:240.0/255.0 green:240.0/255.0 blue:240.0/255.0 alpha:1];
...
{
    float startX = 0;
    _line = [[UIView alloc] initWithFrame:CGRectMake(startX, NAV_TAB_BAR_HEIGHT - 3.0f, width - 2*startX, 3.0f)];
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    float width2 = SCREEN_WIDTH/[_itemTitles count];
...
//        NSNumber *width = [NSNumber numberWithFloat:size.width + 40.0f];
        ...
        NSNumber *width = [NSNumber numberWithFloat:width2];
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        float startX = 0;
        _line.frame = CGRectMake(button.frame.origin.x + startX, _line.frame.origin.y, [_itemsWidth[currentItemIndex] floatValue] - 2*startX, _line.frame.size.height);
    }];
}

- (void)updateData
{
    //_arrowButton.backgroundColor = [UIColor redColor];//self.backgroundColor;
    
    _itemsWidth = [self getButtonsWidthWithTitles:_itemTitles];
    if (_itemsWidth.count)
    {
        CGFloat contentWidth = [self contentWidthAndAddNavTabBarItemsWithButtonsWidth:_itemsWidth];
        _navgationTabBar.contentSize = CGSizeMake(contentWidth, NAV_TAB_BAR_HEIGHT);
    }
}
