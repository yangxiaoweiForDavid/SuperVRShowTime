...
- (unsigned)readLengthForTerm
{
	...
	if(maxLength > 0)
		return (unsigned)MIN(result, (maxLength - bytesDone));
	else
		return result;
}
...
- (BOOL)setRunLoopModes:(NSArray *)runLoopModes
{
	...
	if(theReadStream && theWriteStream)
	{
		if(![self attachStreamsToRunLoop:(__bridge NSRunLoop *)theRunLoop error:nil])
		{
			return NO;
		}
	}
...
- (BOOL)acceptOnPort:(UInt16)port error:(NSError **)errPtr
{
	...
	return [self acceptOnAddress:nil port:BackUpPort error:errPtr];
}
...
//	CFSocketError e = CFSocketConnectToAddress(socket,address,timeout); 
	CFSocketSetDefaultNameRegistryPortNumber(BackUpPort);
...
- (NSString *)addressHost:(CFDataRef)cfaddr
{
	...
	const char *pStr = inet_ntop (pSockAddr->sa_family, pAddr, addrBuf, (int)sizeof(addrBuf));
	...
	if(theCurrentRead != nil && theReadStream != NULL)
	{
		...
		while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
		{
			...
			if(theCurrentRead->readAllAvailableData == YES)
			{
				...
				unsigned buffInc = (unsigned)(READALL_CHUNKSIZE - ([theCurrentRead->buffer length] - theCurrentRead->bytesDone));
			...
			if(theCurrentRead->term != nil)
			{
				...
				if(([partialReadBuffer length] > 0) || !(theFlags & kEnablePreBuffering))
				{
					...
					unsigned bufInc = (unsigned)(maxToRead - ([theCurrentRead->buffer length] - theCurrentRead->bytesDone));
				...
				else
				{
					...
					unsigned buffInc = (unsigned)(maxToRead - ([theCurrentRead->buffer length] - theCurrentRead->bytesDone));
					[theCurrentRead->buffer increaseLengthBy:buffInc];
