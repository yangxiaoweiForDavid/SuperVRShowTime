...
- (NSString *)generateNonce
{
	...
	NSString *newNonce = (__bridge_transfer id)CFUUIDCreateString(NULL, theUUID);
	CFRelease(theUUID);
...
- (BOOL)isAuthenticated
{
	...
	if([self useDigestAccessAuthentication])
	{
		...
		NSURL *absoluteUrl = (__bridge_transfer id)CFHTTPMessageCopyRequestURL(request);
...
- (void)replyToHTTPRequest
{
	...
	NSString *version = (__bridge_transfer id)CFHTTPMessageCopyVersion(request);
	...
	NSURL *uri = (__bridge_transfer id)CFHTTPMessageCopyRequestURL(request);
	...
	NSString *rangeHeader = (__bridge_transfer id)CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Range"));
	...
	else
	{
		...
		else
		{
			...
			if([ranges count] == 1)
			{
				...
				unsigned int bytesToRead = range.length < READ_CHUNKSIZE ? (unsigned int)range.length : READ_CHUNKSIZE;
			...
			else
			{
				...
				unsigned int bytesToRead = (unsigned int)range.length < READ_CHUNKSIZE ? (unsigned int)range.length : READ_CHUNKSIZE;
...
- (CFHTTPMessageRef)prepareMultiRangeResponse:(UInt64)contentLength
{
	...
	CFUUIDRef theUUID = CFUUIDCreate(NULL);
	ranges_boundry = (__bridge_transfer id)CFUUIDCreateString(NULL, theUUID);
	CFRelease(theUUID);
...
- (NSData *)preprocessResponse:(CFHTTPMessageRef)response
{
	...
	NSData *result = (__bridge_transfer id)CFHTTPMessageCopySerializedMessage(response);
	return result;
}
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	if(tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
			...
			NSString *contentLength = (__bridge_transfer id)CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Content-Length"));
			...
			if(expectsUpload)
			{
				...
				uint bytesToRead = (uint)requestContentLength < POST_CHUNKSIZE ? (uint)requestContentLength : POST_CHUNKSIZE;
	...
	else
	{
		...
		if(requestContentLengthReceived < requestContentLength)
		{
			...
			uint bytesToRead = (uint)bytesLeft < POST_CHUNKSIZE ? (uint)bytesLeft : POST_CHUNKSIZE;
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	else if(tag == HTTP_PARTIAL_RANGE_RESPONSE_BODY)
	{
		...
		if(bytesLeft > 0)
		{
			unsigned int bytesToRead = (unsigned int)bytesLeft < READ_CHUNKSIZE ? (unsigned int)bytesLeft : READ_CHUNKSIZE;
