...
/**
 *  陀螺仪模式下视频角度校准。适用于对playerView或者playerView的父视图添加transform的情况
 */
@property (nonatomic,assign) CGFloat transformReviseAngle;
 ...
 *  @return 是否可以切换
 */
-(BOOL)player:(UVPlayer*)player shouldSwitchToAlternateAtIndex:(NSUInteger)index;
/**
 *  不能切换码率失败的原因
 *
 *  @param player 播放器
 ...
 *  @return 原因
 */
-(NSString*)player:(UVPlayer*)player messageForSwitchAlternateAtIndex:(NSUInteger)index;
@end
